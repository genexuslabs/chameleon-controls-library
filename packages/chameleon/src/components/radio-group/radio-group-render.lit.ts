...
const PARTS = (checked: boolean, disabled: boolean) => {
  ...
  return disabled ? `${RADIO_ITEM_PARTS_DICTIONARY.DISABLED} ${checkedValue}` : checkedValue;
};
...
export class ChRadioGroupRender extends KasstorElement {
  ...
  override render() {
    return this.model == null ? nothing : repeat(this.model, item => item.value, this.#itemRender);
  }
}
