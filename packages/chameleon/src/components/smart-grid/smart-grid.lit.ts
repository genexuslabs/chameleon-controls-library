...
import type { AccessibleNameComponent } from "../../utilities/types";
import type { ChameleonControlsTagName } from "../../utilities/types";
import type {
...
export class ChSmartGrid
  ...
  implements AccessibleNameComponent
{
   ...
   */
  @property({ attribute: "auto-grow", type: Boolean }) autoGrow: boolean = false;
   ...
   */
  @property({ attribute: "data-provider", type: Boolean }) dataProvider: boolean = false;
   ...
   */
  @property() threshold: string = "10px";
   ...
   * `"loading"`.
   */
  @Event({ bubbles: false }) protected infiniteThresholdReached!: EventEmitter<void>;
  ...
  #removeAvoidCLS = () => {
    ...
    requestAnimationFrame(() =>
      this.classList.remove(HIDE_CONTENT_AFTER_LOADING_CLASS)
    );
  };
  ...
  override disconnectedCallback(): void {
    ...
    this.removeEventListener("virtualItemsChanged", this.handleVirtualItemsChanged as EventListener);
  }
  ...
  override render() {
    ...
    return html`
      ${initialLoad
        ? html`<slot name="grid-initial-loading-placeholder"></slot>`
        : html`
            ${hasRecords && this.inverseLoading
              ? html`<ch-infinite-scroll
                  ...
                  .infiniteThresholdReachedCallback=${this
                    .#infiniteThresholdReachedCallback}
...
export interface HTMLChSmartGridElement extends ChSmartGrid {}
