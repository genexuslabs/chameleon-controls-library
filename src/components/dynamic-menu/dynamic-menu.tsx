...
/**
 * @part menu-content - The container of ch-dynamic-menu.
 *
...
export class DynamicMenu {
  private menuActions: HTMLChDynamicMenuActionElement[] = [];
  private menuPopups: HTMLChDynamicMenuPopupElement[] = [];
  private openIndex: number = null;

  @Element() el: HTMLChDynamicMenuElement;
  ...
  watchPropOpenItemHandler(newValue: string) {
    ...
    this.menuActions.forEach(
      (action: HTMLChDynamicMenuActionElement, ind: number) => {
  ...
  componentWillLoad() {
    const actions: any = this.el.querySelectorAll("ch-dynamic-menu-action");
  ...
  componentDidLoad() {
    this.menuActions.forEach(action => {
      ...
      if (action.hasAttribute("aria-controls")) {
        const popup: HTMLChDynamicMenuPopupElement = this.el.querySelector(
          `ch-dynamic-menu-popup#${action.getAttribute("aria-controls")}`
        );
  ...
  handleActionKeyDown(event: CustomEvent<KeyboardEvent>) {
    ...
    const targetActionIndex = this.menuActions.indexOf(
      document.activeElement as HTMLChDynamicMenuActionElement
    );
  ...
  private activateMenuPopup(
    popupItem: HTMLChDynamicMenuPopupElement,
    open: boolean
  ) {
