...
export class GridChameleonManagerState {
    ...
    static setColumnSort(columnId: string, sortDirection: ColumnSortDirection): void {
    ...
    private static apply() {
        let sortColumn = this.grid.getColumnByHtmlName((this.state.SortColumnName ?? "").toUpperCase());
        ...
        this.state.Columns?.forEach(stateColumn => {
            let column = this.grid.getColumnByHtmlName(stateColumn.Name.toUpperCase());
            
            if (column) {
              column.Hidden = stateColumn.Hidden ? -1 : 0;
              column.filterEqual = (stateColumn.Filter ?? {}).Equal ?? column.filterEqual;
              column.filterLess = (stateColumn.Filter ?? {}).Less ?? column.filterLess;
              column.filterGreater = (stateColumn.Filter ?? {}).Greater ?? column.filterGreater;
            }
        });
    }

    private static getColumn(name: string): GridChameleonStateColumn {
        let column = this.state.Columns.find(column => column.Name.localeCompare(name) == 0);
