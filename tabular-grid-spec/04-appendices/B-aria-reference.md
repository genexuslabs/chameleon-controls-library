...
| `aria-rowspan` | Cells that span multiple rows | When a cell visually merges across two or more rows | Integer ≥ 2 | Communicates merged row cells to assistive technologies; commonly used in Pivot Table row dimension headers that span child rows | Pivot variant |
| `aria-colspan` | Cells that span multiple columns | When a cell visually merges across two or more columns | Integer ≥ 2 | Communicates merged column cells to assistive technologies; commonly used in Pivot Table column header groups that span child columns | Pivot variant |
